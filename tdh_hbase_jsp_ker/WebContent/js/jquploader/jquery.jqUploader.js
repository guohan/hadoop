 ...
 **/
 jQuery.fn.jqUploader = function(options) {
    return this.each(function(index) {
		...
		var startMessage = ($thisLabel.text() =='') ? '请选择文件' : $thisLabel.text();
		...
		var opts = jQuery.extend({
				...
				errorSizeMessage: '文件太大!',
				validFileMessage: '请点击Upload上传',
				progressMessage: '请等待, 上传中',
				endMessage:    '上传完毕'
		}, options || {}
		);
		...
		TerminateJQUploader = function(containerId,filename,varname){
			...
			$this.text('').append('<input name="'+varname+'" readOnly type="text" id="'+varname+'" value="'+filename+'"/>&nbsp;&nbsp;上传成功');
