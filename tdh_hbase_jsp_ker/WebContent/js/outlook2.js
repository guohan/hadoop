...
function b_addButton(img, label, action , color) {
...
//create one panel
function createPanel(name,caption,showFlag) {
  this.name=name;                  // panel layer ID
  this.ftop=35;                    // actual panel scroll position
  ...
  this.color=new Array();            // button color
  this.showFlag = showFlag			//control if show
  ...
  if(!bt.ns4&&!bt.op5){
  	this.mOver=b_mOver_ie5p;
  	this.mOut=b_mOut_ie5p;
   }
...
function b_writeStyle() {
  ...
  document.write('.button {width:350px;height:'+level_1_height+'px; text-align:center; font-family:arial;');
  document.write(' font-size:'+level_1_fontsize+'pt;color:'+level_1_fontcolor+'; text-decoration: none; cursor:hand;vertical-align: middle;');
  //document.write(' border-style:outset; border-color:silver; ');
  //document.write('background-color:blue;}');
  document.write('background-image: url('+ level_1_img +');background-repeat: no-repeat;');

  document.write('.noLine{text-decoration: none;}');

  document.write('.imgB {color:#1861C2; font-family:arial; font-size:9pt;}');
  ...
  if (bt.op5) {
	...
	document.write(' .txtbin {font-color:#00FF00;  }');
  }
  else {
    document.write('.imgbin {border-width:2; border-style:inset;');
  ...
  if (bt.op5) {
    ...
    document.write('border-color:white;}');
	document.write(' .txtbout {color:#FF0000;  }');
  }
  else {
    document.write('.imgbout {border-width:2; border-style:outset;  ');
    document.write('border-color:#ffffff;}');
  ...
  document.write(' .txtnob {font-color:#000000;  }');
...
var showPanelNo = -1;
  ...
  if (bt.ns4) {                 //draw OutlookBar for ns4
    ...
    for (i=0;i<this.panels.length;i++) {
      document.write('<Layer name='+this.name+'_panel'+i+' width='+this.width);
       document.write(' top='+i*level_1_height+' bgcolor='+this.bgcolor);
       document.write(' clip="0,0,'+this.width+',');
       document.write(this.height-(this.panels.length-1)*level_1_height+'">');

       //one layer to host the panel buttons
       document.write('<Layer top=35 width='+this.width+'>');
        mtop=0
       ...
       document.write(this.width+','+level_1_height+'" bgcolor=silver class=button ');
       ...
       document.write('<Font Color=white class=noLine>'+this.panels[i].caption);
       ...
       document.write('width=16 height=16 src=images/menu/arrowup.gif border=0>');
       document.write('</A></LAYER><Layer top=');
       document.write((this.height-(this.panels.length)*level_1_height)+'<Layer top=');
       document.write((this.height-(this.panels.length)*level_1_height)+' left=');
       ...
       document.write('src=images/menu/arrowdown.gif border=0></A></LAYER>');
    ...
    h=this.height-((this.panels.length-1)*level_1_height)
    ...
    for (i=0;i<this.panels.length;i++) {
	  ...
	  //if panel showflag is true, the panel show be show add by zhangrh
	  //alert("panels[" + i + "]showFlag =="  + this.panels[i].shwoFlag);
	  if (this.panels[i].showFlag!='' && this.panels[i].showFlag == 'true')
	  {
		 showPanelNo = i;
	  }
       ...
       //one layer to host the panel buttons
      document.write('<div id='+this.name+'_panel'+i);
      document.write('_f Style="position:absolute; left:0; top:35; width:');
      ...
      for (j=0;j<this.panels[i].img.length;j++) {
        if (bt.op5) {
          ...
          document.write('Style="position:middle; left:0; width:'+this.width);
          document.write('; top:'+mtop+'; text-align:center;" onclick="" ><table width=100%');
          document.write(' onclick="allbd();bd(\''+this.name+'_panel'+i+'_b'+j+'\');"; ');
          document.write('onmouseOver="img_' + i + '_' + j + '.className=\'imgbout\'; txt_' + i + '_' + j + '.className=\'txtbout\'; b_mOver_ie5p(\''+this.name+'_panel'+i+'_b'+j+'\',top.bottomFrame, 1);" ');
          document.write('onmouseDown="img_' + i + '_' + j + '.className=\'imgbin\';txt_' + i + '_' + j + '.className=\'txtbin\';"');
          document.write('onmouseUp=\'img_' + i + '_' + j + '.className="imgbout";txt_' + i + '_' + j + '.className="txtbout";');
          document.write(this.panels[i].act[j]+';\' ');
          document.write('onmouseOut="img_' + i + '_' + j + '.className=\'imgnob\';txt_' + i + '_' + j + '.className=\'txtnob\';b_mOut_ie5p(\''+this.name+'_panel'+i+'_b'+j+'\',1);"');
		  document.write('><tr><td valign="middle" width=35% align=right>');
          document.write('<img src='+this.panels[i].img[j]+' class=imgnob id="img_' + i + '_' + j + '"');

		  document.write(">");
          document.write('</td><td valign="middle" style="font-size:9pt;color:#000066;"><span id="txt_' + i + '_' + j + '">'+this.panels[i].lbl[j]+'</span></td></tr></table>'+'</DIV>');
        }
        mtop=mtop+this.buttonspace;
		//if panel showflag is true, the panel show be show add by zhangrh
	    //alert("button[" + j + "]color =="  + this.panels[i].color[j]);
		if (this.panels[i].color[j] == 'red')
		{
			bd(this.name+'_panel'+i+'_b'+j);
		}
      }
      ...
      if (!bt.op5) {
        ...
        document.write((this.width-c)+'; height:'+(level_1_height-c)+';"><A href="#" ');
        document.write('onClick="'+this.v+'.showPanel('+i+');this.blur();');
        document.write('return false;" class="noLine" ><table width="100%"  height="100%"><tr><td align="'+level_1_position+'" valign="middle"><FONT color='+level_1_fontcolor+' ');
        document.write(' style="text-decoration:none;font:'+level_1_fontsize+'pt;cursor:hand;filter: Glow(Color:#3B73D7, Strength:3);padding:20px; ">'+this.panels[i].caption);
      ...
      document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
      document.write('<img width=16 height=16 src=images/menu/arrowup.gif border=0 id=uup>');
      ...
      document.write('onmouseOver="'+this.panels[i].v+'.clear();">');
      document.write('<img width=16 height=16 src=images/menu/arrowdown.gif border=0 id=ddown>');
      document.write('</A></DIV>');
    ...
    if (bt.op5) {
      ...
      for (i=0;i<this.panels.length;i++) {
        ...
        document.write(');" style="position:absolute; left:0; top:0; width:');
        document.write((this.width-c)+'; height:'+(level_1_height-c)+';">');
        document.write('<A href="#" ');
        document.write('onClick="'+this.v+'.showPanel('+i+');this.blur();');
        document.write('return false;" class=noLine><FONT color=red ');
      ...
      //document.write(this.height+'; height:350; width:'+this.width);
      //document.write('; background-color:'+this.pagecolor+';"></DIV>');
      //document.write('<DIV style="position:absolute; left:0; top:-350; ');
      //document.write('height:350; width:'+this.width+'; background-color:');
  ...
  //alert("showPanelNo =="  + showPanelNo);
  if (showPanelNo != -1)
  {
	  this.showPanel(showPanelNo);
  }else{
	  /*if (bt.op5) {
...
var l
  ...
  for (i=0;i<l;i++) {
    if (i>nr) {
      this.panels[i].obj.top=this.height-((l-i)*level_1_height)-1;
    }
    else {
      this.panels[i].obj.top=i*level_1_height;
    }
  }
}
...
var l
var o
if(nr==0) document.cookie=nr;
  this.aktPanel=nr;
  l = this.panels.length;
  for (i=0;i<l;i++) {
    if (i>nr) {
      this.panels[i].obj.style.top=this.height-((l-i)*level_1_height);
      ...
      if (bt.op5) {
        ...
        this.panels[i].objc.style.top=this.height-((l-i)*level_1_height);
      }
    }
    else {
      this.panels[i].obj.style.top=i*level_1_height;
      //Opera doesn't support clip:rect()!
      //so show visible panel
      //and move panel caption
      if (bt.op5) {
        this.panels[i].objf.style.visibility='visible';
        this.panels[i].objc.style.top=i*level_1_height;
      }
    }
  }
}
   ...
   if (bt.ns4)
     ...
     if (bt.op5 && (width!=this.width))
       ...
       for (i=0; i<this.panels.length; i++) {
         ...
         this.panels[i].objm2.style.top=(this.height-(this.panels.length)*level_1_height);
...
function createOutlookBar(name,x,y,width,height,bgcolor,pagecolor) {
  ...
  this.buttonspace=38;                   // distance of panel buttons
  ...
  return this
   ...
   function Onwheel(){
	 if(!isNaN(document.all.ddown.length)){
		 if (event.wheelDelta >= 120){
		   if (eval("document.all.Bar_panel"+o.aktPanel+"_m1.style.visibility")=="visible"){
			 document.all.uup[o.aktPanel].click();
		   }
		 } else if (event.wheelDelta <= -120){
		   if (eval("document.all.Bar_panel"+o.aktPanel+"_m2.style.visibility")=="visible"){
			 document.all.ddown[o.aktPanel].click();
		   }
		 }
	 }else{
		 if (event.wheelDelta >= 120){
		   if (eval("document.all.Bar_panel"+o.aktPanel+"_m1.style.visibility")=="visible"){
			 document.all.uup.click();
		   }
		 } else if (event.wheelDelta <= -120){
		   if (eval("document.all.Bar_panel"+o.aktPanel+"_m2.style.visibility")=="visible"){
			 document.all.ddown.click();
		   }
		 }	 
	 }
   }
...
//document.all.Bar_panel0_b0.className="imgB";
for (i=0; i<o.panels.length; i++) {
	for(j=0;j<o.panels[i].img.length;j++){
		var objImg=o.panels[i].img[j];
//		alert(objImg);
//		alert(o.name);
		var idName=o.name+'_panel'+i+'_b'+j;
//		alert(idName);
		eval('document.all.'+idName+'.className="imgB"');
...
function bd(div)
{
	eval('document.all.'+div+'.className="hitred"');

}

function b_mOver_ie5p(nr,bottomFrm,degree){
	var obj=getObj(nr);
    showSubMenu1(obj,"",true,bottomFrm,degree);
}
function b_mOut_ie5p(nr,degree){
    var obj=getObj(nr);
    hideSubMenu1(degree);
}
