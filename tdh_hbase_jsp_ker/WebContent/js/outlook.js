...
function b_writeStyle() {
  ...
  document.write('.button {width:300px;height:25px; text-align:center; font-family:arial;');
  ...
  document.write('background-image: url(images/menu/across.gif);background-repeat: no-repeat;');
...
var c=0;
  ...
  if (bt.ns4) {                 //draw OutlookBar for ns4
    ...
    h=this.height-((this.panels.length-1)*25)
    ...
    if (bt.op5) {
      ...
      for (i=0;i<this.panels.length;i++) {
        ...
        document.write(');" style="position:absolute; left:0; top:0; width:');
        document.write((this.width-c)+'; height:'+(25-c)+';">');
...
var l
  ...
  for (i=0;i<l;i++) {
    if (i>nr) {
      this.panels[i].obj.top=this.height-((l-i)*25)-1;
    }
    else {
      this.panels[i].obj.top=i*25;
    }
  }
}
...
if(nr==0) document.cookie=nr;
  ...
  for (i=0;i<l;i++) {
    if (i>nr) {
      this.panels[i].obj.style.top=this.height-((l-i)*25);
      ...
      if (bt.op5) {
        ...
        this.panels[i].objc.style.top=this.height-((l-i)*25);
      }
    }
    else {
      this.panels[i].obj.style.top=i*25;
      //Opera doesn't support clip:rect()!
      //so show visible panel
      //and move panel caption
      if (bt.op5) {
        this.panels[i].objf.style.visibility='visible';
        this.panels[i].objc.style.top=i*25;
      }
    }
  }
}
