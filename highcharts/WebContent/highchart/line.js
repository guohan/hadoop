...
//    type : 'datetime',
//    minRange : 60 * 1000
	  categories:["报修","报装","咨询","查询","投诉","举报","表扬","建议","意见","其他"]
  },
  yAxis: {
      title: {
          text: '数量'
      },
      tickPositions: [0, 100000, 200000, 300000], // 指定竖轴坐标点的值
      plotLines: [{
          value: 0,
          width: 1,
          color: '#808080'
...
client.connect('user', 'password', function(frame) {
  client.subscribe("/highdatayear", function(message) {
//	  alert(message.body);
...
//    var point = [Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000,Math.random()*100000];
//	  var point=[170, 169, 195, 145, 182, 115, 152, 165, 133, 183];
    var shift = randomData.data.length > 60;
//    , true, shift
//    randomData.addPoint(point);
    randomData.setData(point);
