...
function CalendarInit()				///Create panel
{
	...
	htmloButton+="</div>"
	/// All
	htmlIframe="<iframe frameborder=0 src='javascript:false'style='position:absolute;visibility:inherit; top:0px; left:0px; width:195px; height:195px; z-index:-1; filter='progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)';'></iframe>";
	htmlAll=htmlAll+htmloMonth+htmloYear+htmloDayTable+htmloButton+htmlIframe+"</div>";
...
function getDateString1(oInputSrc,relativeSrc,oInstance)
{
	CalendarTodayClick(oInstance);
	var newDate = new Date();
	var aDate = oInputSrc.value;
	...
	if(oInputSrc&&oInstance) 
	{
		var CalendarDiv=document.all[oInstance.sDIVID];
		oInstance.oTaget=oInputSrc;
		CalendarDiv.style.pixelLeft=CalendargetPos(oInputSrc,"Left")+150;
		CalendarDiv.style.pixelTop=CalendargetPos(relativeSrc,"Top") - relativeSrc.offsetHeight;
		CalendarDiv.style.display=(CalendarDiv.style.display=="none")?"":"none";	
	}	
}
