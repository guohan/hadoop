...
public final class KfkToHiveFilter {
	private static final Pattern SPACE = Pattern.compile(" ");
	private static Logger logger = Logger.getLogger(KfkToHiveFilter.class);
	static String [] jars= {"D:\\sparksql1.jar"};
//	private static final JavaSparkContext sc = new JavaSparkContext(
//			new SparkConf().setMaster("local[*]")
//					.setAppName("kfkToHiveFilter"));
	
	private static final JavaSparkContext sc = new JavaSparkContext(
			new SparkConf().setMaster("yarn-client").setJars(jars).set("SPARK.EXECUTOR.MEMORY", "2g")
					.setAppName("kfkToHiveFilter"));
	...
	public static void main(String[] args) throws IOException {
		...
		InputStream is = KfkToHiveFilter.class.getClassLoader()
		...
		// words.foreachRDD(new Function<String>);
							...
							for (Entry<String, String> entry : sqlList
								...
								if (count > 0) {// 判断结果 入库操作 有数据 不通过 存0 无数据通过存1
									...
									String sql = "Insert into kitdev.RULE_CHECK_RESULT_TODAY"
											...
											+ "'" + "1" + "'" + ")";
//									sqlContext.sql(sql);
//									jdbcDF.toDF().insertInto("tableName");这两种都是数据入库的操作方式 年后回来进行测试 部署到集群
									JDBCUtils.operation(sql);
