...
//    conf.set("hbase.rootdir", "hdfs://nameservice1/hyperbase1");
//	conf.set("hbase.cluster.distributed", "true");
//	 conf.set("hbase.zookeeper.property.clientPort", "2181");
//	conf.set("zookeeper.znode.parent", "/hyperbase1");
    //Login with keytab
...
//    UserGroupInformation.setConfiguration(conf);
//    UserGroupInformation.loginUserFromKeytab("hbase/kit-b1@TDH", HyperbaseDemo.class.getClassLoader().getResource("hbase.keytab").getPath());

    UserGroupInformation.setConfiguration(conf);
	UserGroupInformation.loginUserFromKeytab("kitdev@TDH",
			HyperbaseDemo.class.getClassLoader().getResource("kitdev.keytab")
					.getPath());
    //    //创建表 加载配置
    @SuppressWarnings("resource")
    HBaseAdmin hBaseAdmin = new HBaseAdmin(conf);
    @SuppressWarnings("deprecation")
    HTableDescriptor hTableDescriptor = new HTableDescriptor("t03130");
    hTableDescriptor.addFamily(new HColumnDescriptor("f"));
    hBaseAdmin.createTable(hTableDescriptor);
    System.out.println("create success");
//    HTable table = new HTable(conf, "kit0607");
//    Scan s = new Scan();
...
//    }
//    System.out.println("read in exception!");
//    table.close();
  }
}
