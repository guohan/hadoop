...
var dataListing = jQuery("#dataListing");
...
//    randomData.addPoint(point, true, shift);
		  ...
		  var areaOutput='';
			jQuery.each(data.areaList, function(i,vh) {
				 areaOutput +="<tbody><tr><td>"+ vh.color+"</td><td>"+vh.status+"</td><td>"+vh.area+"</td><td>"+vh.type+"</td></tr></tbody>";
			});
			var a_tabl_start = "<table class='table table-bordered table-condensed table-hover innerTable'><thead><tr><th>color</th><th>status</th><th>area</th><th>type</th></tr></thead>";
			var a_tabl_end = "</table>";
			dataListing.html(a_tabl_start+areaOutput+a_tabl_end);
  });
