...
var Status=new function()
{
    ...
    this.setMainInfo=function(_msg,_needRefresh)
    {
        ...
        this.mainInfo.innerHTML+=(_needRefresh)?"<br>若系统长时间停止响应，请<a href='javascript:document.location.reload(true);'>刷新</a>〄17 ":"";
...
/**
 * @author zxub
 * 用于存放通道名称及17信对象的类，这样可以17过不同通道名称来区分不同的通信对象
 */
...
var Request=new function()
{
    ...
    this.createInstance=function()
    {
        ...
        if (window.XMLHttpRequest)
        {
            ...
            //有些版本的Mozilla浏览器处理服务器返回的未包含XML mime-type头部信息的内容时会出错17因此，要确保返回的内容包含text/xml信息
     ...
     * @param _chunnel：17道名称，若不存在此参数，则默认丄17"default"
     * @return 丄17个17信对象，其存放于17信类缓存中
     */
    this.getInstance=function(_chunnel)
    {
        var instance=null;
        var object=null;
        if (_chunnel==undefined)//没指定17道名称
        {
            _chunnel="default";
        }
        var getOne=false;
     ...
     * @param _url:请求目的
     * @param _data:要发送的数据
     * @param _processRequest:用于处理返回结果的函数，其定义可以在别的地方，需要有丄17个参数，即要处理的17信对象
     * @param _chunnel:通道名称，默认为"default"
     * @param _asynchronous:是否异步处理，默认为true,即异步处琄17
     */
    this.send=function(_url,_data,_processRequest,_chunnel,_asynchronous)
    {    
        if (_url.length==0 || _url.indexOf("?")==0)
        {
            Status.showErrInfo("ajax请求处理","<font color=red>由于目的为空，请求失败，请检查！</font>","5秒后该提示自动消处17");
            window.setTimeout("Status.setStatusShow(false)",5000);
            return;
        }
        if (this.showStatus)
        {
            Status.setMainInfo("正在与服务器交换数据，请稍1717...");  
        }
        ...
        if (instance==null)
        {
            Status.showErrInfo("ajax请求处理","<font color=red>浏览器不支持ajax，请棄17查！</font>","5秒后该提示自动消处17");
            window.setTimeout("Status.setStatusShow(false)",5000);
            return;
        }  
        ...
        if (_asynchronous==true && typeof(_processRequest)=="function")
        {
            ...
            {
                if (instance.readyState == 4) // 判断对象状1717
                {
                    if (instance.status == 200) // 信息已经成功返回，开始处理信恄17
                    {
                        try
                    	{
                        ...
                        {
                            Status.showErrInfo("ajax回调处理时发生错误：","<font color=red>"+e.message+"</font>","5秒后该提示自动消处17");
                            window.setTimeout("Status.setStatusShow(false)",5000);
                        }
                    }
                    else
                    {
                        Status.showErrInfo("ajax请求处理","<font color=red>您所请求的页面有异常，请棄17查！</font>","5秒后该提示自动消处17");
                        window.setTimeout("Status.setStatusShow(false)",5000);
                    }
                }
            }
        ...
        //_url加一个时刻改变的参数，防止由于被浏览器缓存后同样的请求不向服务器发17请汄17
        if (_url.indexOf("?")!=-1)
        {
    ...
    /**
     * 间隔丄17段时间持续发送请求，只用于异步处理，只用于GET方式
     * @param _interval:请求间隔，以毫秒讄17
     * @param _url:请求地址
     * @param _processRequest:用于处理返回结果的函数，其定义可以在别的地方，需要有丄17个参数，即要处理的17信对象
...
{   
    ...
    /**
     * 该函数的效果是在应用它的script块后加一个script坄17
...
/**
 * ajax调用远程页面后，远程页面中script块未执行的处琄17
 */
