...
(function (a) {
	a.fn.dragsort = function (c) {
		...
		var f = null, e = null;
		if (this.selector) {
			a("head").append("<style type='text/css'>" + (this.selector.split(",").join(" " + d.dragSelector + ",") + " " + d.dragSelector) + " { cursor: pointer; list-style-type:none;}</style>");
		}
		this.each(function (h, g) {
			if (a(g).is("table") && a(g).children().size() == 1 && a(g).children().is("tbody")) {
				g = a(g).children().get(0);
			}
			var j = {draggedItem:null, placeHolderItem:null, pos:null, offset:null, offsetLimit:null, container:g, init:function () {
				...
				f.draggedItem.css({position:"absolute", opacity:0.8, "z-index":999,"width":150}).after(d.placeHolderTemplate);
				...
				this.draggedItem.css({top:l, left:k});
				...
				f.draggedItem.css({position:"", top:"", left:"", opacity:"", "z-index":""});
	...
	a.fn.dragsort.defaults = {itemSelector:"li", dragSelector:"li", dragSelectorExclude:"input, a[href]", dragEnd:function () {
