...
//        HTable table = (HTable) pool.getTable(tableName); 
    ...
    public static void insertData(String tableName,String topic,String message) throws IOException { 
        System.out.println("start insert data ......"); 
//        HTablePool pool = new HTablePool(configuration, 1000); 
//        HTable table = (HTable) pool.getTable(tableName); 
        
        HTable table = new HTable(configuration, tableName);  
        Put put = new Put((new Date()).toString().getBytes());// 一个PUT代表一行数据，再NEW一个PUT表示第二行数据,每行一个唯一的ROWKEY，此处rowkey为put构造方法中传入的值 
        put.add("column1".getBytes(), null, (new Date()).toString().getBytes());// 本行数据的第一列 
        put.add("column2".getBytes(), null, topic.getBytes());// 本行数据的第三列 
        put.add("column3".getBytes(), null, message.getBytes());// 本行数据的第三列 
        try { 
            table.put(put); 
            table.close();
        } catch (IOException e) { 
