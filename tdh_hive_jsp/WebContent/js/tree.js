...
function tree(objName,path) {
	...
	this.icon = {
		...
		root				: path + 'images/tree/tree_base.gif',

		folder			: path + 'images/tree/tree_folder.gif',

		folderOpen	: path + 'images/tree/tree_folderopen.gif',

		node				: path + 'images/tree/tree_folder.gif',

		empty				: path + 'images/tree/tree_empty.gif',

		line				: path + 'images/tree/tree_line.gif',

		join				: path + 'images/tree/tree_join.gif',

		joinBottom	: path + 'images/tree/tree_joinbottom.gif',

		plus				: path + 'images/tree/tree_plus.gif',

		plusBottom	: path + 'images/tree/tree_plusbottom.gif',

		minus				: path + 'images/tree/tree_minus.gif',

		minusBottom	: path + 'images/tree/tree_minusbottom.gif',

		nlPlus			: path + 'images/tree/tree_nolines_plus.gif',

		nlMinus			: path + 'images/tree/tree_nolines_minus.gif'
	};
...
// Add by bocen.li Aug 19, 2007 Begin

tree.prototype.getNode = function(onId)
{
	var n = 0;
	for(n; n < this.aNodes.length;n++)
	{
		if(this.aNodes[n].id == onId && this.aNodes[n].url)
		{
...
tree.prototype.getCurNodeUrl = function(onId)
{
	var cnode = this.getNode(onId);
	if(cnode!=null)
		return cnode.url;
	else
		return "";
...
tree.prototype.getCurNodeTarget = function(onId)
{
	var cnode = this.getNode(onId);
	if(cnode!=null)
		return cnode.target;
...
tree.prototype.toSelectIndex = function(pNode, idLevel, bShowNotContain)
{
	var n = 0;
	for(var i=idLevel.options.length-1; i>=0; i--)
	{
		idLevel.options.remove(i);
	}
	var newOption = document.createElement("OPTION");
	newOption.text = "请选择";
	newOption.value = "-999";
	idLevel.options.add(newOption, idLevel.options.length);
	for(n; n < this.aNodes.length;n++)
	{
		if(this.aNodes[n].pid == pNode.id)
		{
			newOption = document.createElement("OPTION");
			newOption.text = this.aNodes[n].name;
			newOption.value = this.aNodes[n].id;
			idLevel.options.add(newOption, idLevel.options.length);
		}		
	}
	if(idLevel.options.length > 0) 
	{
		idLevel.selectedIndex = 0;
		idLevel.style.display = "block";
	}else
	{
		idLevel.style.display = "block";
	}	
...
// Add by bocen.li Aug 19, 2007 End
