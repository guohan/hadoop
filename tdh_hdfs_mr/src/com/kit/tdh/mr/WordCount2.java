...
/**
 * 
 * @author GH 此方法不用另外生成jar 在代码中可以直接调试
 *
 */
...
//		    ("hdfs/kit-b1@TDH",
//		    		WordCount.class.getClassLoader().getResource("hdfs.keytab").getPath());
		String[] otherArgs = new String[2];

		otherArgs[0] = "hdfs://kit-b1:8020/user/discover/fpm.tx";// 计算原文件目录，需提前在里面存入文件

//		String time = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());

		otherArgs[1] = new Path("hdfs://kit-b1:8020/user/discover/output201741e12311").toString();
//				+ time;// 计算后的计算结果存储目录，每次程序执行的结果目录不能相同，所以添加时间标签
		...
		Job job = new Job(conf, "word_count_mapreduce_on_linux");

		job.setJarByClass(WordCount.class);

		// "D:\\a_hadoop_test/hadoop.jar"
